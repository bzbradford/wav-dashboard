...
if (has$baseline) {
  ...
  buildReportBaselineTable(baseline) %>%
    kable(caption = "Baseline stream monitoring measurements") %>%
    print()

  if (has$streamflow) {
    buildReportStreamflowTable(baseline) %>%
  ...
  buildReportFieldworkComments(baseline) %>%
    paste(collapse = "\n\n") %>%
    cat()
}
...
if (has$thermistor) {
  ...
  thermistor %>%
    buildReportPlot(type = "thermistor") %>%
    print()
