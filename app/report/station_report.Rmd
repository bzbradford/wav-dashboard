...
```{r setup, include=F}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, error = TRUE, results = "asis")
...
if (has$water_temp) {
  ...
  interp$baseline_temp <- str_glue("***Based on the maximum observed water temperature of {max_temp_f}°F ({max_temp_c}°C), this is likely to be a {temp_class} stream.***")
}
...
if (has$thermistor) {
  ...
  max_temp_f <- signif(c_to_f(max_temp_c), 3)
  temp_class <- get_temp_class(max_temp_f)
  interp$therm_temp <- str_glue("***Based on the maximum mean daily water temperature of {max_temp_f}°F ({max_temp_c}°C), this is likely to be a {temp_class} stream.***")
}
...
if (has$d_o) {
  ...
  interp$d_o <- str_glue("***Based on the minimum recorded dissolved oxygen of {min_do} mg/L, this stream can support {do_class}.***")
}
...
if (has$ph) {
  ...
  interp$ph <- str_glue("***Based on recorded pH values, this stream {ph_class}.***")
}
...
if (has$specific_cond) {
  ...
  interp$specific_cond <- str_glue("***Based on the recorded maximum specific conductance measurement of {max_cond} µS/cm, this stream {cond_class}.***")
}
...
if (has$streamflow) {
  ...
  interp$streamflow <- str_glue("***Based on the minimum recorded streamflow of {min_flow} cfs, this is likely to be a {flow_class}.***")
}
```
