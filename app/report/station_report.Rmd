...
report_summary <- build_report_summary(params)
has <- report_summary$has
```

```{r interp, include=FALSE}
...
if (has$water_temp) {
  max_temp_c <- signif(max(baseline$water_temp, na.rm = TRUE), 3)
  max_temp_f <- signif(c_to_f(max_temp_c), 3)
  temp_class <- get_temp_class(max_temp_f)
  interp$baseline_temp <- str_glue(
    "***Based on the maximum observed water temperature of {max_temp_f}°F ({max_temp_c}°C), this is likely to be a {temp_class} stream.***"
  )
}

if (has$thermistor) {
  max_temp_c <- signif(
    max(summarize(thermistor, temp_c = mean(temp_c), .by = date)$temp_c),
    3
  )
  max_temp_f <- signif(c_to_f(max_temp_c), 3)
  temp_class <- get_temp_class(max_temp_f)
  interp$therm_temp <- str_glue(
    "***Based on the maximum mean daily water temperature of {max_temp_f}°F ({max_temp_c}°C), this is likely to be a {temp_class} stream.***"
  )
}

if (has$d_o) {
  min_do <- min(baseline$d_o, na.rm = TRUE)
  do_class <- case_when(
    ...
    TRUE ~ "limited to no aquatic life"
  )
  interp$d_o <- str_glue(
    "***Based on the minimum recorded dissolved oxygen of {min_do} mg/L, this stream can support {do_class}.***"
  )
}

if (has$ph) {
  min_ph <- min(baseline$ph, na.rm = TRUE)
  max_ph <- max(baseline$ph, na.rm = TRUE)
  ph_class <- case_when(
    min_ph < 6 &&
      max_ph >
        9 ~ "was both too acidic and too basic for water quality standards",
    min_ph < 6 ~ "was too acidic for water quality standards",
    max_ph > 9 ~ "was too basic for water quality standards",
    TRUE ~ "meets water quality standards"
  )
  interp$ph <- str_glue(
    "***Based on recorded pH values, this stream {ph_class}.***"
  )
}

if (has$specific_cond) {
  max_cond <- max(baseline$specific_cond, na.rm = TRUE)
  cond_class <- case_when(
    ...
    TRUE ~ "did not have elevated conductivity levels"
  )
  interp$specific_cond <- str_glue(
    "***Based on the recorded maximum specific conductance measurement of {max_cond} µS/cm, this stream {cond_class}.***"
  )
}

if (has$streamflow) {
  min_flow <- min(baseline$streamflow, na.rm = TRUE)
  ...
  interp$streamflow <- str_glue(
    "***Based on the minimum recorded streamflow of {min_flow} cfs, this is likely to be a {flow_class}.***"
  )
}
```
...
```{r}
knitr::include_graphics("header.png", error = FALSE, dpi = 300)
```

\begin{center}
\Large{\textbf{`r params$year` WAV Fieldwork Report - `r stn$station_name`}}
\end{center}

```{r stn-map, fig.width=7, fig.height=3.5}
build_report_map(stn)
```
...
# Baseline observation summary

if (has$baseline) {
  p("# Baseline Monitoring Summary")
  build_report_baseline_table(baseline) |>
    kable(caption = "Baseline stream monitoring measurements") |>
    print()

  if (has$streamflow) {
    build_report_streamflow_table(baseline) |>
      kable(caption = "Streamflow details") |>
      print()
  }

  baseline |>
    summarize_report_cols(c(report_baseline_cols, report_streamflow_cols)) |>
    kable(caption = "Baseline stream monitoring summary statistics") |>
    print()

  p("## Additional fieldwork details")
  build_report_fieldwork_comments(baseline) |>
    paste(collapse = "\n\n") |>
    cat()
}
...
if (has$air_temp || has$water_temp) {
  ...
  baseline |>
    build_report_plot("temp") |>
    print()
  baseline |>
    mutate(across(c(air_temp, water_temp), c_to_f)) |>
    summarize_report_cols(c(
      `Air temp (°F)` = "air_temp",
...
if (has$thermistor) {
  ...
  thermistor |>
    build_report_plot(type = "thermistor") |>
    print()
  p(
    "_On the chart above, Natural Community temperature classifications are shown as shaded bands._"
  )
...
if (has$ph) {
  ...
  p("## Why measure pH?")
  p(
    "pH is a measure of the hydrogen-ion activity of water and is expressed as a logarithmic value that ranges from 1 to 14. That means for every 1 pH change, there is a tenfold change in the activity of hydrogen ions. Waters with high hydrogen-ion activity have low pH and are considered acidic. The presence of dissolved carbon dioxide, carbonic acid, bicarbonate ions and carbonate ions strongly influence the pH of freshwater systems. Wisconsin has adopted a pH standard that incorporates a range from 6 to 9 units to protect and support aquatic life."
  )
  p(
    "Much of Wisconsin has bedrock that is composed of limestone and other rock that is rich in bicarbonate and carbonate. These types of rock can minimize impacts of acid rain or other acidic inputs to the water. In northern Wisconsin, bedrock is composed of granite which is not rich in bicarbonates and carbonates, and thus those rocks are less able to buffer or offset the impacts of acid rain or other acid inputs to the waters."
  )
  p(
    "Concentrations of dissolved metal ions tend to increase with increased acidity and as a result, pH is an important factor influencing toxicity of metals. pH also affects the concentration of un-ionized ammonia, a form of nitrogen that is extremely toxic to aquatic life."
  )
  p(interp$ph)
}
...
if (has$nutrient) {
  phos_estimate <- get_phos_estimate(nutrient$tp)
  phos_exceedance_text <- get_phos_exceedance_text(phos_estimate)

  pg()
  p("# Total Phosphorus Measurements")
  nutrient |>
    build_report_plot(type = "nutrient") |>
    print()
  nutrient |>
    summarize_report_cols(c(
