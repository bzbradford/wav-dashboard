...
nlcd_classes <- read_csv("nlcd_classes.csv") %>%
...
landscape_data %>%
  filter(huc_level == 12) %>%
  filter(huc %in% sample(.data$huc, 100)) %>%
  ggplot(aes(y = pct_area, x = paste(huc, huc_name), fill = paste(class, class_name))) +
  geom_col(width = 1) +
...
test_landscape_diff <- test_landscape_all %>%
  left_join({
    test_landscape %>%
      select(class_name, pct_area2 = pct_area)
    }) %>%
