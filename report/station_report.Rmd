...
# adjust titlesec heading spacing on wisc connect server
if (Sys.getenv("SERVER") == "wisc") {
  titlespacing <- c(1.5, 0)
} else {
  titlespacing <- c(3, 1.5)
}
...
if (has$specific_cond) {
  ...
  cond_class <- case_when(
    max_cond > 2000 ~ "reached potentially toxic chloride levels",
    max_cond > 1500 ~ "approached potentially toxic chloride levels",
    max_cond > 800 ~ "had higher than average conductivity levels",
    T ~ "meets water quality standards"
  )
  interp$specific_cond <- glue("***Based on recorded the maximum specific conductivity measurement of {max_cond} µS/cm, this stream {cond_class}.***")
}
...
\titlespacing*{\section}{0pt}{`r titlespacing[1]`ex}{`r titlespacing[2]`ex}
\titlespacing*{\subsection}{0pt}{`r titlespacing[1]`ex}{`r titlespacing[2]`ex}
...
* *Confidence interval (CI):* A confidence interval (the 80% confidence interval in the case of total phosphorus monitoring) represents range such that we are 80% confident (or some other confidence, such as 90%) that the true mean value that we are measuring lies between the upper and lower boundaries of that range. When making repeated observations, natural variation will result in different observations. More observations, or a lower level of natural variation, will result in tighter confidence intervals around a mean.

## Automatic data interpretations

Some of the pages in this report include a brief interpretation of the data, which is based on the specific criteria outlined below. Always be sure to inspect the data with a critical eye and keep in mind that there may be data entry errors that can result in invalid data being included on the dashboard and in these reports. It is also important to review several years of data from a stream to develop a better understanding of the stream's health and natural community classification.

* **Water temperature:** Both water temperature and streamflow need to be reviewed together to assign a Natural Community classification to a stream. For water temperature, the DNR uses four categories: *Coldwater* streams have maximum daily average temperatures below 69.3°F (20.7°C), *cool-cold* streams have maximum average temperatures between 69.3°F and 72.5°F (22.5°C), *cool-warm* streams have maximum average temperatures between 72.5°F and 76.3°F (24.6°C), and *warmwater* streams have maximum average temperatures above 76.3°F.
* **Dissolved oxygen:** Coldwater fish communities during spawning season (fall-spring) require a minimum of 7 mg/L, coldwater communities during other times of year require a minimum of 6 mg/L, and warmwater communities require a minimum of 5 mg/L. Few fish can survive when DO drops to 3 mg/L, and virtually no aquatic life can survive DO levels of 1 mg/L or less.
...
* **Streamflow:** Ephemeral streams have minimum flows <0.03 cfs, headwater streams between 0.03 and 3 cfs, mainstem streams between 3 and 150 cfs, and large rivers have minimum flow greater than 150 cfs. Technically these values are for the 90% exceedance flow (90% of streamflow rates greater than this value), but most volunteers collect less than 10 streamflow values per year so the minimum streamflow is used here.
* **Total phosphorus:** The DNR established a phosphorus criteria of 0.075 mg/L for streams. For a stream to be listed as impaired, 6 monthly samples must be collected in each of two or more different years, and the entire 80% confidence interval for the median phosphorus value in each year must exceed the state standard. We use the following language for characterizing phosphorus levels: A stream site ***clearly exceeds*** the phosphorus criteria if the lower confidence limit exceeds the state standard. A stream site ***may exceed*** the phosphorus criteria if the median is higher than the standard but the lower confidence interval does not exceed the standard. A stream site ***may meet*** the phosphorus criteria if the median is below than the standard, but the upper confidence interval remains above the standard. When the median and the entire confidence interval is below the phosphorus standard, the stream ***clearly meets*** the state phosphorus criteria.
...
if (has$thermistor) {
  ...
  p("_On the chart above, the cutoffs for a stream's temperature classification are shown as shaded areas._")
