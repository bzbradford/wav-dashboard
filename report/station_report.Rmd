...
Some of the pages in this report include a brief interpretation of the data, which is based on the criteria listed below and developed by the Wisconsin DNR. Always be sure to inspect the data with a critical eye and keep in mind that there may be data entry errors that can result in invalid data being included on the dashboard and in these reports.

* **Water temperature:** `r water_temp_class_text` For baseline sampling, we use the maximum single value. For hourly temperature logger data (where available), we use the maximum daily average temperature.
