...
`r buildReportSummary(params$year, params$stn, params$data)`
...
if (has_baseline) {
  ...
  has_temp <- with(baseline, sum(!is.na(ambient_air_temp)) + sum(!is.na(water_temp)) > 0)
  if (has_temp) {
    pg()
    p("# Temperature")
  ...
  has_do <- with(baseline, sum(!is.na(d_o)) > 0)
  if (has_do) {
    pg()
    p("# Dissolved Oxygen")
  ...
  has_trans <- with(baseline, sum(!is.na(transparency_average)) > 0)
  if (has_trans) {
    pg()
  ...
  has_flow <- with(baseline, sum(!is.na(streamflow_cfs)) > 0)
  if (has_flow) {
    pg()
    p("# Streamflow")
...
has_nutrient <- with(nutrient, sum(!is.na(tp)))
if (has_nutrient) {
  ...
  pg()
  p("# Nutrient monitoring summary")
  makeReportPlots(nutrient, type = "nutrient") %>% print()
...
has_therm <- n_distinct(thermistor$date) > 0
if (has_therm) {
  ...
  makeReportPlots(thermistor, type = "thermistor") %>% print()
  p(sprintf("***Based on the maximum daily average water temperature of %s°C (%s°F), this is a %s stream.***", signif(max_temp, 3), signif(c_to_f(max_temp), 3), stream_type))
