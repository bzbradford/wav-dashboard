...
counts <- list(
  ...
  nutrient = length(na.omit(nutrient$tp)),
  thermistor = n_distinct(thermistor$date)
)
...
\lhead{`r str_trunc(stn$label, 65)`}
...
Baseline monitoring: `r counts$baseline` fieldwork events\
Nutrient monitoring: `r counts$nutrient` monthly samples\
Temperature logger: `r counts$thermistor` days deployed
